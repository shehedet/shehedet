[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=100&pause=1000&color=F7F7F7&repeat=false&width=435&lines=%3E+.%2Fshehedet%40GitHub)](https://git.io/typing-svg)
```text
             
         :@=.                 .-@@:                 [1;36mname:[0m A. K. M. Shahadat Hossain
         :@@@+.             :=@@@@:                 role: CSE@BRAC University
         :@@@@@-:-@@@@@@@@:--@@@@@.                 year_of_graduation: 2026
         .=@@###@@@@@@@@@@@@##@@@=:                 
         :@=:  ..*#@@@@@@#+:  .=@=.                 languages: [Python, JavaScript, C++]
        -=@@@#+++-*******=-+==##@#-.                markup_languages: [HTML, CSS]
       -*==@@###**--   --**+##@@@++-                databases: [MySQL, MongoDB]
       -+**++#*##@@@- -@@@@*+=+**=-                 machine_learning_frameworks: [PyTorch, TensorFlow, Scikit-Learn]
    ..-:*####*++*##**=+*##***#####*-:.              data_science_frameworks: [NumPy, Pandas, Matplotlib]
 ...:.-**+****++++++++*+++*#*****+==--:. .          web_development_frameworks: [React.js, Node.js, Express.js]
...:=++**++*####%###%+#######*+====:---:-
:=+*#*++==***+==++++##+***++====+++-::::::
*+===-:----:::---=-+=+=++++++++++-:--...::

```
